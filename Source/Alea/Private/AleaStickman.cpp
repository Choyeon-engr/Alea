...
void AAleaStickman::MulticastLive_Implementation()
{
...
void AAleaStickman::Die(AActor* DamageCauser)
{
	...
	GetWorldTimerManager().SetTimer(ReleaseTimer, FTimerDelegate::CreateLambda([this]() -> void { Release(); }), 1.f, false);
}
...
void AAleaStickman::Attack()
{
	...
	if (AutoAttackCoolDownTime <= 0.f)
	{
		...
		else
		{
			...
			MulticastRangedAttack(AttackEffect, nullptr, HitEffect, this, TrgtAutoAttacked, AttackDamage);
		}
